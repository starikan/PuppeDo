...
class Envs {
    ...
    runBrowsers() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < envsNames.length; i += 1) {
                ...
                if (!['api', 'browser', 'electron'].includes(type) ||
                ...
                if (type === 'browser') {
                    if (runtime === 'run') {
                        if (engine === 'puppeteer') {
                            const { browser, pages } = yield Envs.runPuppeteer(browserSettings);
                            env.state = Object.assign(Object.assign({}, env.state), { browser, pages });
                        }
                        if (engine === 'playwright') {
                            const { browser, pages } = yield Envs.runPlaywright(browserSettings);
                            env.state = Object.assign(Object.assign({}, env.state), { browser, pages });
                        }
                    }
                }
                if (type === 'electron') {
                    if (runtime === 'connect') {
                        const { browser, pages } = yield Envs.connectElectron(browserSettings);
                        env.state = Object.assign(Object.assign({}, env.state), { browser, pages });
                    }
                    if (runtime === 'run') {
                        const { browser, pages, pid } = yield this.runElectron(browserSettings, env);
                        env.state = Object.assign(Object.assign({}, env.state), { browser, pages, pid });
                    }
                }
            }
            this.runBrowsers = () => { };
    ...
    static runPuppeteer(browserSettings) {
        return __awaiter(this, void 0, void 0, function* () {
            const { PPD_DEBUG_MODE = false } = new Arguments();
            const { headless = true, slowMo = 0, args = [] } = browserSettings;
            const browser = yield puppeteer.launch({ headless, slowMo, args, devtools: PPD_DEBUG_MODE });
            const page = yield browser.newPage();
            const pages = { main: page };
            const { width, height } = _.get(browserSettings, 'windowSize');
            if (width && height) {
                yield pages.main.setViewport({ width, height });
            }
            return { browser, pages };
        });
    }
    static runPlaywright(browserSettings) {
        return __awaiter(this, void 0, void 0, function* () {
            const { PPD_DEBUG_MODE = false } = new Arguments();
            ...
            const browser = yield playwright[browserName].launch(options);
            const context = yield browser.newContext();
            const page = yield context.newPage({ viewport: { width, height } });
            const pages = { main: page };
            const contexts = { main: context };
            return { browser, contexts, pages };
        });
    }
    static connectElectron(browserSettings) {
        return __awaiter(this, void 0, void 0, function* () {
            const urlDevtoolsJson = _.get(browserSettings, 'urlDevtoolsJson');
            if (urlDevtoolsJson) {
                const jsonPagesResponse = yield fetch(`${urlDevtoolsJson}json`, { method: 'GET' });
                const jsonBrowserResponse = yield fetch(`${urlDevtoolsJson}json/version`, { method: 'GET' });
                const jsonPages = yield jsonPagesResponse.json();
                const jsonBrowser = yield jsonBrowserResponse.json();
                ...
                const pagesRaw = yield browser.pages();
                let pages = {};
                if (pagesRaw.length) {
    ...
    runElectron(browserSettings, env) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (runtimeExecutable) {
                process.env = Object.assign(Object.assign({}, process.env), browserEnv);
                ...
                yield sleep(pauseAfterStartApp);
                const { browser, pages } = yield Envs.connectElectron(browserSettings);
    ...
    static resolveLinks() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const allData = yield new TestsContent();
            // ENVS RESOLVING
            args.PPD_ENVS = args.PPD_ENVS.map((v) => {
                ...
                if (env) {
                    ...
                    envsExt.forEach((d) => {
                        const envsResolved = Object.assign({}, allData.envs.find((g) => g.name === d, {}));
                    ...
                    dataExt.forEach((d) => {
                        const dataResolved = Object.assign({}, allData.data.find((g) => g.name === d, {}));
                        env.data = merge(_.get(env, 'data', {}), _.get(dataResolved, 'data') || {}, data);
                    });
                    selectorsExt.forEach((d) => {
                        const selectorsResolved = Object.assign({}, allData.selectors.find((g) => g.name === d, {}));
    ...
    init(runBrowsers = true) {
        return __awaiter(this, void 0, void 0, function* () {
            const args = Object.assign({}, (yield Envs.resolveLinks()));
