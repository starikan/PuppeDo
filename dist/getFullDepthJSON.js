...
const getFullDepthJSON = ({ testName, testBody = {}, levelIndent = 0, envsId = null } = {}) => {
    ...
    const testJSON = _.cloneDeep(allTests.allContent.find((v) => v.name === testNameResolved && ['atom', 'test'].includes(v.type)));
    if (!testJSON) {
        throw new Error(`Test with name '${testNameResolved}' not found in root folder and additional folders`);
    }
    const fullJSON = _.cloneDeep(Object.assign(Object.assign({}, testJSON), testBody));
    ...
    RUNNER_BLOCK_NAMES.forEach((runnerBlock) => {
        ...
        if (_.isArray(runnerBlockValue)) {
            ...
            throw new Error(`Running block '${runnerBlock}' in test '${fullJSON.name}' in file '${fullJSON.testFile}' \
...
//# sourceMappingURL=getFullDepthJSON.js.map