...
const paintString = (str, color = 'sane') => `\u001b[${colors[color] || 0}m${str}\u001b[0m`;
const blankSocket = {
  send: () => {
    // Do nothing
  },
...
const getTimer = (timeStart = process.hrtime.bigint()) => {
...
const initOutputLatest = () => {
  ...
  external_fs_default().copyFileSync(resolveOutputHtmlFile(), external_path_default().join(folderLatest, 'output.html'));
  return {
    folderLatest,
...
const initOutput = envsId => {
  ...
  external_fs_default().copyFileSync(resolveOutputHtmlFile(), external_path_default().join(folder, 'output.html'));
...
class Screenshot {
  ...
  static async copyScreenshotToLatest(pathScreenshot) {
    const {
      folderLatest = '.'
    } = new Environment().getOutput();
...
class Log {
  constructor(envsId, loggerOptions = {}) {
    const {
      stdOut
    } = loggerOptions;
    this.envsId = envsId;
  ...
  async log({
    ...
    const {
      log
    } = new Environment().getEnvAllInstance(this.envsId);
    const {
      textColor = 'sane',
    ...
    try {
      const logEntries = texts.map(textString => {
        var _this$options$testArg, _this$options, _this$options2, _this$options2$testAr;
        const logEntry = {
          text: textString,
          ...
          time: new Date(),
          screenshots,
          stepId: ((_this$options$testArg = this.options.testArgs) === null || _this$options$testArg === void 0 ? void 0 : _this$options$testArg.stepId) || stepId,
          funcFile,
          testFile,
          extendInfo,
          error,
          ...
          breadcrumbs: ((_this$options = this.options) === null || _this$options === void 0 ? void 0 : _this$options.breadcrumbs) || [],
          repeat: ((_this$options2 = this.options) === null || _this$options2 === void 0 ? void 0 : (_this$options2$testAr = _this$options2.testArgs) === null || _this$options2$testAr === void 0 ? void 0 : _this$options2$testAr.repeat) || 1
        };
        return logEntry;
      });
      ...
      err.stepId = (_this$options$testArg2 = this.options.testArgs) === null || _this$options$testArg2 === void 0 ? void 0 : _this$options$testArg2.stepId;
      throw err;
    }
  }
}
...
function utils_merge(/* obj1, obj2, obj3, ... */) {
  ...
  const assignValue = (val, key) => {
    if (isPlainObject(result[key]) && isPlainObject(val)) {
      result[key] = utils_merge(result[key], val);
    } else if (isPlainObject(val)) {
      result[key] = utils_merge({}, val);
    } else if (isArray(val)) {
...
class Engines {
  ...
  static async runElectron(browserSettings, envName, outputs) {
    const {
      runtimeEnv = {}
    } = browserSettings;
    const {
    ...
    if (runtimeExecutable && folder && folderLatest) {
      ...
      if (prc) {
        external_fs_default().writeFileSync(external_path_default().join(folder, `${envName}.log`), '');
        external_fs_default().writeFileSync(external_path_default().join(folderLatest, `${envName}.log`), '');
        prc.stdout.on('data', data => {
          external_fs_default().appendFileSync(external_path_default().join(folder, `${envName}.log`), String(data));
          external_fs_default().appendFileSync(external_path_default().join(folderLatest, `${envName}.log`), String(data));
        });
      }
  ...
  static async connectElectron(browserSettings) {
    ...
    if (urlDevtoolsJson) {
      const jsonPagesResponse = await node_modules_axios(`${urlDevtoolsJson}json`, {
        method: 'GET'
      });
      const jsonBrowserResponse = await node_modules_axios(`${urlDevtoolsJson}json/version`, {
        method: 'GET'
      });
    ...
    if (type === 'electron') {
      ...
      if (runtime === 'run') {
        const {
          ...
          pages,
          pid
        } = await Engines.runElectron(browserSettings, this.name, outputs);
        newState = {
          browser,
          pages,
          pid
...
class Environment extends Singleton {
  ...
  createEnv(data = {}) {
    ...
    if (!this.instances[envsId]) {
      ...
      for (const loggerPipe of (_loggerOptions$logger = loggerOptions === null || loggerOptions === void 0 ? void 0 : loggerOptions.loggerPipes) !== null && _loggerOptions$logger !== void 0 ? _loggerOptions$logger : []) {
        var _loggerOptions$logger;
...
class ContinueParentError extends AbstractError {
  ...
  async log() {
    ...
    await this.logger.log({
      level: 'warn',
      levelIndent,
      text: `Continue: ${((_this$parentError = this.parentError) === null || _this$parentError === void 0 ? void 0 : _this$parentError.message) || `test with expr ${breakParentIfResult}'`}`
    });
  }
}
...
const logExtendFileInfo = async (log, levelIndent, envsId = '') => {
  if (envsId) {
...
const logErrorMessage = async (log, levelIndent, error) => {
  if (error.message) {
...
const logStack = async (log, levelIndent, error, stdOut = false) => {
  if (error.stack) {
...
const logTimer = async (log, startTime, levelIndent = 0) => {
  const {
    PPD_LOG_EXTEND
  } = new Arguments().args;
  if (PPD_LOG_EXTEND) {
...
const logExtend = async (log, levelIndent, args, isError = false) => {
  const {
    PPD_LOG_EXTEND
  } = new Arguments().args;
  const {
...
const logArgs = async (log, levelIndent, stdOut = false) => {
...
const logDebug = async (log, levelIndent, args, stdOut = false, type = true) => {
  let text = [];
  const {
    data,
    selectors
...
const checkIf = async (expr, ifType, log, levelIndent = 0, allData = {}, logShowFlag = true, continueOnError = false) => {
  ...
  constructor(initValues) {
    ...
    this.runLogic = async inputs => {
      ...
      if (PPD_TAGS_TO_RUN.length && this.tags.length && !this.tags.filter(v => PPD_TAGS_TO_RUN.includes(v)).length) {
        await logger.log({
          text: `Skip with tags: ${JSON.stringify(this.tags)} => ${getLogText(this.description, this.name, PPD_LOG_TEST_NAME)}`,
          level: 'raw',
          levelIndent: this.levelIndent,
          logShowFlag,
          logOptions: {
      ...
      try {
        ...
        // IF
        if (this.if) {
          const skipIf = await checkIf(this.if, 'if', logger.log.bind(logger), this.levelIndent, allData, logShowFlag, this.plugins.getValue('continueOnError').continueOnError);
          if (skipIf) {
            return {
              result: {},
              meta: {}
        ...
        // ERROR IF
        if (this.errorIf) {
          await checkIf(this.errorIf, 'errorIf', logger.log.bind(logger), this.levelIndent, allData, logShowFlag, this.plugins.getValue('continueOnError').continueOnError);
        }
        ...
        if (this.errorIfResult) {
          await checkIf(this.errorIfResult, 'errorIfResult', logger.log.bind(logger), this.levelIndent + 1, {
            ...allData,
            ...localResults
          }, logShowFlag, this.plugins.getValue('continueOnError').continueOnError);
        }
        ...
        // TIMER IN CONSOLE
        if ((PPD_LOG_EXTEND || PPD_LOG_TIMER_SHOW) && (!PPD_LOG_NAMES_ONLY.length || PPD_LOG_NAMES_ONLY.includes(this.name))) {
          await logger.log({
...
const makeLog = ({
  ...
  if (level === 'error' && !extendInfo) {
    breadcrumbs.forEach((v, i) => {
      stringsLog.push([{
        text: `${nowWithPad}${indentString}${'   '.repeat(i)} ${v}`,
        textColor: 'error'
      }]);
    });
    if (testFile) {
      stringsLog.push([{
        text: `${nowWithPad}${indentString} (file:///${external_path_default().resolve(testFile)})`,
        textColor: 'error'
      }]);
    }
    if (funcFile) {
      stringsLog.push([{
        text: `${nowWithPad}${indentString} (file:///${external_path_default().resolve(funcFile)})`,
        textColor: 'error'
      }]);
    }
  }
...
const formatterEntry = async logEntry => makeLog(logEntry);
...
const fileLog = (envsId, texts = [], fileName = 'output.log') => {
  const {
    folderLatest = '.',
    folder = '.'
  } = new Environment().getOutput(envsId);
  let textsJoin = '';
  ...
  external_fs_default().appendFileSync(external_path_default().join(folder, fileName), `${textsJoin}\n`);
  external_fs_default().appendFileSync(external_path_default().join(folderLatest, fileName), `${textsJoin}\n`);
};
...
class TestStructure {
  ...
  static resolveRunner(runnerValue, runnerNum, fullJSONIncome, runnerBlock, levelIndent) {
    const runner = Object.entries(runnerValue);
  ...
  static getFullDepthJSON(testName, testBody = {}, levelIndent = 0) {
    ...
    RUNNER_BLOCK_NAMES.forEach(runnerBlock => {
      ...
      runnerBlockValue.forEach((runnerValue, runnerNum) => {
        const fullJSONResponce = TestStructure.resolveRunner(runnerValue, runnerNum, fullJSON, runnerBlock, levelIndent);
        if (fullJSONResponce) {
...
const runTest = async (testName, envsId) => {
  ...
  const {
    logger
  } = new Environment().getEnvAllInstance(envsId);
  await logger.log({
    level: 'timer',
  ...
  await logger.bulkLog([{
    ...
    level: 'timer',
    text: `Prepare time 🕝: ${getTimer(startTimeTest).delta}`
  }]);
  const testResults = await test();
  await logger.log({
    level: 'timer',
    text: `Test '${testName}' time 🕝: ${getTimer(startTimeTest).delta}`
  });
  return testResults;
};
...
async function run(argsInput = {}, optionsInit = {}) {
  ...
  try {
    ...
    await logger.log({
      level: 'timer',
      text: `Evaluated time 🕝: ${getTimer(startTime).delta}`
    });
