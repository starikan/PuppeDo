...
```
## allureSuite
Плагин генерации отчета для Allure. Отчет создается для конкретного тега, шагами являются все вложенные теги

### Доступные параметры

| Parameter | Default | Description |
| --- | :---: | --- |
| allureSuite | true | Дескриптор для создания отчёта Allure для данного |

### Доступные аргументы

| Parameter | Default | Description |
| --- | :---: | --- |
| PPD_ALLURE_TAG_ON | [] | Автоматическое создание отчётов для конкретных тегов, без явного прописывания дескриптора |
