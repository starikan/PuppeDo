...
### 9.3 Создание собственных атомов
...
### 9.4 Структура плагина

Плагины — это расширения, которые добавляют новые поля или изменяют поведение агентов. Плагин состоит из TypeScript файла:
...
> 💡 **Разница между атомами и плагинами:**
> - **Атом** — это действие, которое вызывается по имени (`- myAtom:`)
> - **Плагин** — это расширение, которое добавляет новое поле/поведение ко ВСЕМ агентам
...
> ⚠️ **Важно: Data и Selectors под капотом**
>
> При выполнении агента `data` и `selectors` **объединяются в один контекст**. Это означает:
> - Ключи из `data` и `selectors` доступны в одном пространстве имён
> - Если ключ есть и в data, и в selectors с **разными значениями** — будет ошибка
> - По сути это одна и та же структура, разделённая для семантического удобства
>
> ```yaml
> # Эти данные объединятся в один контекст:
> data:
>   url: "https://example.com"
> selectors:
>   button: "#submit"
> # В выражениях доступны оба: url и button
> ```
...
Все переменные задаются в `puppedo.config.js`:
