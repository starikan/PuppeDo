...
Для этого используются два поля:
- `stepId` — уникальный идентификатор шага
- `stepIdNext` — переход к шагу по его stepId (поддерживает как статические значения, так и JavaScript-выражения)
...
`stepIdNext` указывает, какой шаг выполнить **следующим** (вместо следующего по порядку). Поддерживает как статические значения (строки), так и JavaScript-выражения.
...
**Динамический переход (через выражение):**

`stepIdNext` может вычислять следующий шаг **во время выполнения** через JavaScript-выражение.
...
**Контекст выражения:** В `stepIdNext` доступны все данные (`data`), селекторы (`selectors`) и результаты предыдущих шагов.
