...
describe('TestTree', () => {
  describe('getTree', () => {
    ...
    test('getTree returns the correct tree after creating a top-level step and a child step', () => {
      ...
      expect(tree.getTree()).toEqual([
        { stepId: 'step1', timeStart: expect.any(Date), steps: [{ stepId: 'step2', timeEnd: expect.any(Date) }] },
      ]);
    });
  });
  ...
  describe('createStep', () => {
    ...
    test('creates a top-level step when stepIdParent is null', () => {
      ...
      const stepId = '1';
      testTree.createStep({ stepIdParent: null, stepId, payload });
      expect(testTree.getTree()).toEqual([{ stepId, ...payload }]);
    });
    ...
    test('adds a new step to the steps array of a parent step when stepIdParent is not null', () => {
      ...
      testTree.createStep({ stepIdParent: null, stepId: parentStepId, payload: parentPayload });
      testTree.createStep({ stepIdParent: parentStepId, stepId: childStepId, payload: childPayload });
      expect(testTree.getTree()).toEqual([
        { stepId: parentStepId, ...parentPayload, steps: [{ stepId: childStepId, ...childPayload }] },
      ]);
    });
